...
class TestEvidenceSearchAgent:
    ...
    async def test_execute_with_evidence_queue(
        self, evidence_agent: EvidenceSearchAgent
    ) -> None:
        """証跡キューありでの実行テスト"""
    ...
    def test_classify_evidence_type_sharepoint(
        self, evidence_agent: EvidenceSearchAgent
    ) -> None:
        """SharePoint証跡タイプ分類テスト"""
        # PDFドキュメント
        assert evidence_agent._classify_evidence_type(
            {"name": "承認記録.pdf", "mime_type": "application/pdf"}, "sharepoint"
        ) == "pdf_document"

        # Excel
        assert evidence_agent._classify_evidence_type(
            {"name": "データ.xlsx", "mime_type": ""}, "sharepoint"
        ) == "spreadsheet"

        # Word
        assert evidence_agent._classify_evidence_type(
            {"name": "契約書.docx", "mime_type": ""}, "sharepoint"
        ) == "word_document"

    def test_classify_evidence_type_sap(
        self, evidence_agent: EvidenceSearchAgent
    ) -> None:
        """SAP証跡タイプ分類テスト"""
        assert evidence_agent._classify_evidence_type(
    ...
    def test_classify_evidence_type_email(
        self, evidence_agent: EvidenceSearchAgent
    ) -> None:
        """メール証跡タイプ分類テスト"""
        assert evidence_agent._classify_evidence_type(
            {"has_attachments": True}, "email"
        ) == "email_with_attachment"
        assert evidence_agent._classify_evidence_type(
            {"has_attachments": False}, "email"
        ) == "email"
