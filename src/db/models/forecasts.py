...
class RiskForecast(TenantBaseModel):
    ...
    feature_importance: Mapped[dict[str, Any] | None] = mapped_column(JSONB, default=dict)
    contributing_factors: Mapped[list[Any] | None] = mapped_column(JSONB, default=list)  # 寄与要因リスト
...
class CrossCompanyPattern(TenantBaseModel):
    ...
    source_tenant_ids: Mapped[list[str] | None] = mapped_column(ARRAY(String), nullable=True)  # 匿名化ID
    pattern_data: Mapped[dict[str, Any] | None] = mapped_column(JSONB, default=dict)  # パターン詳細
    benchmark_scores: Mapped[dict[str, Any] | None] = mapped_column(JSONB, default=dict)  # 業種ベンチマークスコア
    risk_correlation: Mapped[dict[str, Any] | None] = mapped_column(JSONB, default=dict)  # リスク相関マトリクス
