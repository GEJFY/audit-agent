...
class AuditeeResponse(TenantBaseModel):
    ...
    evidence_ids: Mapped[list | None] = mapped_column(ARRAY(String), nullable=True)
...
class RiskAlert(TenantBaseModel):
    ...
    detection_data: Mapped[dict | None] = mapped_column(JSONB, default=dict)
...
class ControlsStatus(TenantBaseModel):
    ...
    threshold_config: Mapped[dict | None] = mapped_column(JSONB, default=dict)  # 閾値設定
...
class SelfAssessment(TenantBaseModel):
    ...
    questionnaire: Mapped[dict | None] = mapped_column(JSONB, default=dict)  # 質問・回答一覧
    overall_score: Mapped[float | None] = mapped_column(Float, nullable=True)
    risk_areas: Mapped[list | None] = mapped_column(JSONB, default=list)
...
class PrepChecklist(TenantBaseModel):
    ...
    items: Mapped[list | None] = mapped_column(JSONB, default=list)  # [{task, status, assignee, due_date}]
    predicted_questions: Mapped[list | None] = mapped_column(JSONB, default=list)  # AI予測質問
    pre_collected_evidence: Mapped[list | None] = mapped_column(ARRAY(String), nullable=True)
